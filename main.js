...
class Game {
	...
	get globalCpsMultiplier() {
		return (this.frenzy.active ? this.frenzy.mult : 1);
	}
...
const UI = {
	init(game) {
		...
		if (this.$saveBtn) this.$saveBtn.addEventListener("click", () => Storage.save(this.game));
...
renderLists() {
		...
		if (this.$upgradeList) this.$upgradeList.innerHTML = ""; // upgrades verwijderd
		...
		if (this.$upgradeList) this.$upgradeList.addEventListener("click", (e) => this.handleBuy(e));
